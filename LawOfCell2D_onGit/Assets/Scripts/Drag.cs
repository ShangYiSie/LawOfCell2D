...
public class Drag : MonoBehaviour
{
    ...
    void OnMouseUp()
    {
        ...
        if (trigger == true)
        {
            ...
            // ChangeCardCollider.enabled = false;
            ...
            PlayerObj.GetComponent<Player>().GetParameter(tempatk, tempdfs, tempCardConsume, tempAddMp, tempCardbuff, tempCarddebuff);
            #endregion

            #region 換置Sprite及Drag腳本
            temp.gameObject.GetComponent<SpriteRenderer>().sprite = Card.GetComponent<CardCreat>().CardsImg[0];
            Destroy(temp.GetComponent<CardParameter>());
            temp.gameObject.AddComponent<Drag>();
            this.GetComponent<Animation>().Play();
            // temprandom = UnityEngine.Random.Range(1, CardCreat.CardSum);
            // this.GetComponent<SpriteRenderer>().sprite = Card.GetComponent<CardCreat>().CardsImg[temprandom];
            temprandom = UnityEngine.Random.Range(0, CardCreat.CardSum);
            this.GetComponent<SpriteRenderer>().sprite = Card.GetComponent<CardCreat>().CardsImg[Level.PlayerCardS[temprandom]];
            this.gameObject.AddComponent<CardParameter>();
            this.GetComponent<CardParameter>().DefineParameter(Level.PlayerCardS[temprandom]);
            this.transform.position = oldpos;
            this.gameObject.AddComponent<LongPress>();
            Destroy(this.GetComponent<Drag>());
            #endregion
            PlayerObj.gameObject.GetComponent<Player>().PlayerAction();
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (Player.isboss)
        {
            Enemy.EnemyCD = Boss.EnemyCD;
        }
        if (other.tag != "longpress" && PlayerState.IsName("Player_idle") && EnemyState.IsTag("idle") && Enemy.EnemyCD != 0 && Player.isDrag == true)
        {
